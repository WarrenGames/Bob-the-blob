...
void saveDemo::saveData(const fs::path& demoFilePath, const demos::DataPackage& dataPackage, std::ofstream& demoFile)
{
	...
	saveDemo::saveDuration(demoFilePath, dataPackage.enemyBobVulnerabilityDelay, demoFile, demosData::EnemyBobVulnerabilityDelay);
	...
	saveDemo::saveGameEvents(demoFilePath, dataPackage.greenOrbRelatedItemsData.greenOrbsAndPlayer, demoFile, demosData::GreenOrbPlayerPowerSwitching);
	saveDemo::saveDuration(demoFilePath, dataPackage.greenOrbRelatedItemsData.greenOrbPowerDuration, demoFile, demosData::GreenOrbPowerDuration);
...
void saveDemo::saveDuration(const fs::path& demoFilePath, long long int duration, std::ofstream& demoFile, int fileIndex)
{
	if( ! ( demoFile << fileIndex << " " << duration << '\n' ) )
	{
		saveDemo::throwWriteError(demoFilePath, fileIndex);
	}
}
