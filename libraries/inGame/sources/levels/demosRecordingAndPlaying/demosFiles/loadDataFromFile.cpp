...
void loadDemo::actAccordingToIndex(const fs::path& demoFilePath, demos::DataPackage& dataPackage, std::istringstream& lineStream, std::size_t fileLineNumber, int lineIndex)
{
	switch( lineIndex )
	{
		...
		case demosData::EnemyBobsRespawn:
			...
			break;
		case demosData::EnemyBobVulnerabilityDelay:
			loadDemo::loadDurationData(demoFilePath, lineStream, dataPackage.enemyBobVulnerabilityDelay, fileLineNumber);
			break;
		...
		case demosData::GreenOrb:
			...
			break;
		case demosData::GreenOrbPlayerPowerSwitching:
			loadDemo::loadGameEvents(demoFilePath, lineStream, dataPackage.greenOrbRelatedItemsData.greenOrbsAndPlayer, fileLineNumber);
			break;
		case demosData::GreenOrbPowerDuration:
			loadDemo::loadDurationData(demoFilePath, lineStream, dataPackage.greenOrbRelatedItemsData.greenOrbPowerDuration, fileLineNumber);
			break;
...
void loadDemo::loadDurationData(const fs::path& demoFilePath, std::istringstream& lineStream, long long int& durationData, std::size_t fileLineNumber)
{
	if( ! ( lineStream >> durationData ) )
	{
		loadDemo::throwReadError(demoFilePath, fileLineNumber);
	}
}
